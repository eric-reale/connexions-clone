...
var circleSectionDiv = document.querySelector('.my-circles');
var circleNameInput = document.querySelector('.new-circle-input');
...
function createCircleHTML(array) {
  ...
  var html = fixArray.map(function (circle) {
...
function removeConnexionFromCircle(e, circle) {
  e.preventDefault();
  var connexionToRemove = e.target.parentElement;
  var connexionID = connexionToRemove.children[1].dataset.id;

  _axios2.default.post('/connexions/updateConnexionCircles', {
    id: connexionID,
    circle: circle
  }).then(function (res) {
    // console.log(res.data)
...
function singlePageTab(e) {
  var otherTab = e.currentTarget.parentElement.parentElement.previousElementSibling;
  if (otherTab === null) {
    otherTab = e.currentTarget.parentElement.parentElement.nextElementSibling;
  };

  var otherActiveTab = otherTab.childNodes[1].firstElementChild;
  otherActiveTab.classList.remove('active');
  e.currentTarget.classList.add('active');

  var profileSection = document.querySelector('.profile-active');
  var chapterSection = document.querySelector('.chapter-active');
...
var circleNames = void 0;
window.addEventListener('DOMContentLoaded', function () {
  if (document.title === 'Add to my Circles | Connexions!') {
    circleNames = (0, _displayExistingCircles2.default)(circles);
  } else {
    return;
  }
});
...
window.addEventListener('DOMContentLoaded', function () {
  var removeFromCircle = document.querySelectorAll('.remove-line-circle');
  removeFromCircle.forEach(function (toRemove) {
    toRemove.addEventListener('click', function (e) {
      return (0, _removeConnexionFromCircle2.default)(e, circle);
    });
  });
});
